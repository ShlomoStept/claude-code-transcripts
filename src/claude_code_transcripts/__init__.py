...
def render_content_block(block):
    ...
    if block_type == "image":
        ...
        if isinstance(content, str):
            ...
            content_markdown_html = "".join(parts) if parts else format_json(content)
        elif is_json_like(content):
            content_markdown_html = format_json(content)
        else:
            content_markdown_html = format_json(content)
        return _macros.tool_result(
            content_markdown_html, content_json_html, is_error, has_images
        )
    else:
